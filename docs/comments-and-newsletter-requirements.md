# コメント機能 + ニュースレター拡張 要件定義

最終更新: 2026-05-05
ステータス: ドラフト v2（マージ済みニュースレター機能を反映）
...
直近マージされた **ニュースレター機能（PR #1, commit 9677fb6）** をベースにし、

- ニュースレター登録に **ユーザー名（表示名）** を加える拡張
- ニュースレター登録者向けの **マジックリンクログイン基盤** の追加
...
要件定義の前提として、現状（main ブランチ）のニュースレター実装を整理する。
...
ニュースレターの `confirm_token` とは独立した、**繰り返し利用可能なログイン用トークン**を新設する。
...
| 未登録メールでログイン要求 | 「このメールアドレスはニュースレターに登録されていません」と表示し、登録フォームへ誘導 |
...
| 表示位置 | 記事本文下、関連記事より上 |
| 投稿フォーム位置 | コメントリストの上端（未ログイン時はリスト下にニュースレター登録 CTA + ログイン CTA） |

### 5.8 通知

| トリガー | 通知方法 | デフォルト |
|---|---|---|
| ニュースレター登録確認 | メール（既存） | 必須 |
| マジックリンク発行 | メール（新規） | 必須 |
...
すべて **Supabase (Postgres)** に集約する（既存ニュースレター実装と整合）。
...
RLS は既存ニュースレターと同様に **deny all** とし、Next.js API ルートが `SUPABASE_SERVICE_ROLE_KEY` でアクセスする。
...
- フェーズ分けせず **一括リリース**
- リリース判定基準:
  - 表示名つきニュースレター登録 → 確認 → ログイン → コメント投稿 → 返信 → いいね → 編集 → 削除 → 退会、のフルフローが本番環境で動作
  - オーナー BAN フローが動作
