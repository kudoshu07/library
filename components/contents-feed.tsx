...
const relatedLinks: RelatedLinkItem[] = [
  ...
  { kind: "source", source: "ig_photo", href: "https://www.instagram.com/onoshu_photo/", external: true },
  {
    kind: "source",
    source: "pod_gorilla",
    href: "https://open.spotify.com/show/033DjojO61aLYQoJGhbQlx",
    external: true,
  },
  {
    kind: "source",
  ...
  return (
    ...
    >
      ...
      <input
        ...
        className="h-11 w-full rounded-full border border-slate-200 bg-slate-50 pl-10 pr-10 text-sm text-slate-900 outline-none transition focus:border-[#264F8B] focus:bg-white"
                  ...
                  <nav className="mt-6 space-y-2" aria-label="Main menu">
                    <Link
                      ...
                      className={`flex items-center gap-3 rounded-full px-3 py-2 text-base font-semibold transition min-[800px]:max-[1024px]:mx-auto min-[800px]:max-[1024px]:h-10 min-[800px]:max-[1024px]:w-10 min-[800px]:max-[1024px]:justify-center min-[800px]:max-[1024px]:px-0 min-[800px]:max-[1024px]:py-0 ${
                        isHomeNavActive ? "text-[#264F8B] hover:bg-slate-200" : "text-slate-900 hover:bg-slate-200"
                      }`}
                    >
                ...
                {ENABLE_SUBSCRIBE_UI && (
                  isLoggedIn ? (
                    ...
                    >
                      ...
                      <span className="min-[800px]:max-[1024px]:hidden">{subscribeButtonLabel}</span>
                    </button>
                  )
                )}
              </div>
                  ...
                  <div className="flex justify-end">
                    {isLoggedIn ? (
                      ...
                      >
                        ...
                        <span className="whitespace-nowrap">{subscribeButtonLabel}</span>
                      </button>
                    )}
                  </div>
                </div>
            ...
            {pickupItems.length > 0 && (
              <section className="border-b border-slate-200 px-4 py-4 min-[800px]:hidden">
                <div className="mb-3 flex items-center gap-2">
                  <Sparkles className="size-4 text-[#264F8B]" />
              ...
              <div className="flex gap-2 overflow-x-auto pb-1">
                {sourceFilterOptions.map((source) => (
                  <button
                    ...
                    className={
                      isFilterActive(source.value)
                        ? "shrink-0 rounded-full bg-[#264F8B] px-4 py-2 text-sm font-semibold text-white"
        ...
        <div className={ENABLE_SUBSCRIBE_UI ? "grid grid-cols-4" : "grid grid-cols-3"}>
          <Link
            ...
            className={`flex h-[clamp(3.75rem,8svh,4.5rem)] flex-col items-center justify-center gap-1 px-1 text-[10px] font-semibold leading-none transition sm:text-xs ${
              isHomeNavActive ? "text-[#264F8B]" : "text-slate-700"
            }`}
          >
            <Home className="size-5" />
            ホーム
